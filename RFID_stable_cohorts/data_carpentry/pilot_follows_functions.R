...
new_pairs <- function(vs, window = 500) {
  result <- data.frame(element1 = integer(), 
                       element2 = integer(), 
                       number_from_element1 = integer(), 
                       number_from_element2 = integer())
  
  num_elements <- length(vs)
  
  # Iterate through each element in the list
  for (i in seq_len(num_elements - 1)) {
    # Iterate through each number in the current element
    for (j in seq_along(vs[[i]])) {
      current_number <- vs[[i]][j]
      # Iterate through subsequent elements
      for (k in seq(from = (i + 1), to = num_elements)) {
        # Iterate through numbers in subsequent elements
        for (l in seq_along(vs[[k]])) {
          if (l > length(vs[[k]])) break  # Stop if index is out of bounds
          compared_number <- vs[[k]][l]
          diff <- abs(current_number - compared_number)
          if (diff <= window) {
            result <- rbind(result, 
                            data.frame(element1 = i, 
                                       element2 = k, 
                                       number_from_element1 = current_number, 
                                       number_from_element2 = compared_number))
          } else if (compared_number > current_number + window) {
            break  # Stop comparing if the difference exceeds the window
          }
        }
      }
    }
