...
mouse1_zone <- mouse1_zone %>%
  mutate(duration=ms-lag(ms))
 
mouse1_zone %>%
  mutate(duration=ms-lag(ms)) %>%
  group_by(zone) %>%
  summarize(total=sum(!is.na(duration)))
...
## observation: all 4 mice spend the most time in zone 1 
```
...
mouse1_zone[unlist(m1_rows),]

## how to discard the middle row of each set of 3 rows
