...
# 
# Match <- function(row) {
#   subset(df, deviceid == row$deviceid & 
...
# 
# 
# f <- function(x) df[c(sapply(1:nrow(x), function(i) c(x$ms[i], Match(x[i, ])))), ]
# lapply(split(df, df$data), f)

ddf <- df[1:100,c('ms','deviceid','antennaID','data')]
colnames(ddf)<- c('time', 'device', 'antenna',   'ID')

ddf$time <- as.integer(ddf$time)
...
f <- function(x) ddf[c(sapply(1:nrow(x), function(i) c(x$time[i], Match(x[i, ])))), ]


lapply(split(ddf, ddf$ID), f)
